...
-- Стандартные размеры частей тела (для корректного восстановления)
local DEFAULT_SIZES = {
...
function Hitbox:Initialize(Tab)
    ...
    self.Connection   = nil
    -- [userId][partName] = Vector3 оригинального размера
    self.originalSizes = {}
    ...
    HitboxGroup:AddDropdown("HitboxTargetPart", {
        ...
        Text    = "Target Part",
        Callback = function(v)
            -- При смене части — восстанавливаем старые размеры, потом меняем
...
-- Возвращает список частей тела для данного персонажа согласно TargetPart
...
-- Сохраняет оригинальный размер части (только при первом вызове)
...
-- Возвращает сохранённый оригинальный размер или дефолт
...
function Hitbox:Start()
    self:Stop() -- Отключаем старый коннект перед стартом
...
function Hitbox:Stop()
    ...
    -- Восстанавливаем все изменённые части
    for _, player in pairs(Players:GetPlayers()) do
